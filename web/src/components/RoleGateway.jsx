...
export default function RoleGateway({ onLogin }) {
  ...
  // Registration is two steps: fill in details (sends an email OTP), then
  // enter the code to actually create the account.
  ...
  async function submitSchool(e) {
    ...
    try {
      if (schoolMode === 'login') {
        const res = await schoolLogin(schoolEmail, schoolPassword);
      ...
      if (registerStep === 'form') {
        const res = await schoolRegisterRequestOtp({ email: schoolEmail, schoolName, branch, location, password: schoolPassword });
        if (!res?.success) {
          setError(res?.error || 'Could not send a verification code.');
          return;
        }
        setOtpNote(`We sent a 6-digit code to ${schoolEmail}. Enter it below to finish creating your school account.`);
        setOtpCode('');
        setRegisterStep('otp');
        return;
      }

      const res = await schoolRegisterVerifyOtp(schoolEmail, otpCode);
      if (!res?.success || !res?.token) {
  ...
  async function resendSchoolOtp() {
    setBusy(true);
    setError('');
    try {
      const res = await schoolRegisterRequestOtp({ email: schoolEmail, schoolName, branch, location, password: schoolPassword });
      ...
      setOtpNote(`New code sent to ${schoolEmail}.`);
    } finally {
      setBusy(false);
    }
  }
        ...
        {role === 'school' ? (
          <form className="rg-form" onSubmit={submitSchool}>
            <div className="rg-inline-toggle">
              <button
                type="button"
                className={schoolMode === 'register' ? 'active' : ''}
                onClick={() => { setSchoolMode('register'); setRegisterStep('form'); setError(''); setOtpNote(''); }}
              >Register School</button>
              <button
                type="button"
                className={schoolMode === 'login' ? 'active' : ''}
                onClick={() => { setSchoolMode('login'); setRegisterStep('form'); setError(''); setOtpNote(''); }}
              >School Login</button>
            </div>

            {schoolMode === 'register' && registerStep === 'otp' ? (
              <>
                {otpNote ? <p className="rg-otp-note">{otpNote}</p> : null}
                <label>
                  Verification Code
                  <input
                    value={otpCode}
                ...
                <button className="rg-submit" type="submit" disabled={busy || otpCode.length !== 6}>
                  {busy ? 'Please wait...' : 'Verify & Create Account'}
                </button>
                <div className="rg-otp-actions">
                  <button type="button" className="rg-link-btn" disabled={busy} onClick={resendSchoolOtp}>Resend code</button>
                  <button type="button" className="rg-link-btn" disabled={busy} onClick={() => { setRegisterStep('form'); setError(''); setOtpNote(''); }}>← Change details</button>
                </div>
              </>
            ) : (
              <>
                ...
                <button className="rg-submit" type="submit" disabled={busy}>
                  {busy ? 'Please wait...' : (schoolMode === 'register' ? 'Send Verification Code' : 'Login as School Admin')}
                </button>
              </>
            )}
          </form>
