...
class TeacherApiService {
  ...
  /// `{id, name, className, email}` list, optionally filtered — mirrors
  /// `getTeacherStudents({q, className})` in `web/src/api.js`.
  Future<List<Map<String, dynamic>>> getStudents({
    String search = '',
    String className = '',
  }) async {
    final json = await _get('/teacher/students', {
      if (search.trim().isNotEmpty) 'q': search.trim(),
      if (className.trim().isNotEmpty && className.toLowerCase() != 'all')
  ...
  /// One student's full homework list with status/grade/attempt info —
  /// mirrors `getTeacherStudentHomework(studentId)` in `web/src/api.js`.
  Future<List<Map<String, dynamic>>> getStudentHomework(
    String studentId,
  ) async {
    final json = await _get(
      '/teacher/students/${Uri.encodeComponent(studentId)}/homework',
      const {},
    );
  ...
  /// Grades one homework submission — mirrors `gradeTeacherHomework(hwId, payload)`
  /// in `web/src/api.js` / `POST /teacher/homework/:hwId/grade`.
  Future<Map<String, dynamic>> gradeHomework(
    String hwId, {
    int? grade,
    String? feedback,
  }) {
    return _post('/teacher/homework/${Uri.encodeComponent(hwId)}/grade', {
      'status': 'graded',
      'grade': grade,
      'feedback': feedback,
    });
  ...
  /// One student's progress for the teacher's own subject — same shape as one
  /// entry of the student portal's Learning Score `subjects[]`, scoped down to
  /// the teacher's subject — mirrors `getTeacherStudentSubjectProgress(studentId)`
  /// in `web/src/api.js`.
  Future<Map<String, dynamic>?> getStudentSubjectProgress(
    String studentId,
  ) async {
    final json = await _get(
      '/teacher/students/${Uri.encodeComponent(studentId)}/subject-progress',
      const {},
    );
    final subject = json['subject'];
    return subject is Map ? Map<String, dynamic>.from(subject) : null;
  }

  /// The student's full "My Learning Report" (every subject, same as what the
  /// student sees on their own Progress page) — mirrors
  /// `getTeacherStudentLearningScore(studentId)` in `web/src/api.js` /
  /// `GET /teacher/students/:id/learning-score`.
  Future<LearningScoreData> getStudentLearningScore(String studentId) async {
    final json = await _get(
      '/teacher/students/${Uri.encodeComponent(studentId)}/learning-score',
      const {},
    );
  ...
  Future<Map<String, dynamic>> updateAnnouncement(
    ...
    return _post('/teacher/announcements/${Uri.encodeComponent(id)}/update', {
      'title': title,
      'message': message,
      'className': className,
      'startAt': startAt,
  ...
  Future<void> deleteAnnouncement(String id) =>
      _delete('/teacher/announcements/${Uri.encodeComponent(id)}');
  ...
  Future<Map<String, dynamic>> updateHomework(
    ...
    return _post(
      '/teacher/homework/${Uri.encodeComponent(homeworkId)}/update',
      {
        'title': title,
        'subject': subject,
        'note': note,
        'attachmentUrls': attachmentUrls,
        'attachmentUrl': attachmentUrls.isNotEmpty
  ...
  Future<void> deleteHomework(String homeworkId) =>
      _delete('/teacher/homework/${Uri.encodeComponent(homeworkId)}');
  ...
  Future<void> deleteTest(String testId) =>
      _delete('/tests/${Uri.encodeComponent(testId)}');
}
